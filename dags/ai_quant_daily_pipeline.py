...
with DAG(
    ...
    start_parallel >> ingest_crypto >> etl_crypto >> train_crypto >> signal_crypto >> audit_crypto >> quality_crypto >> reconcile_crypto >> end_parallel
    start_parallel >> ingest_stocks >> etl_stocks >> train_stocks >> signal_stocks >> audit_stocks >> quality_stocks >> reconcile_stocks >> end_parallel

    (
        end_parallel
        >> ingest_midcap >> etl_midcap >> train_midcap >> signal_midcap >> audit_midcap >> quality_midcap >> reconcile_midcap
        >> ingest_smallcap >> etl_smallcap >> train_smallcap >> signal_smallcap >> audit_smallcap >> quality_smallcap >> reconcile_smallcap
        >> ingest_mf >> etl_mf >> train_mf >> signal_mf >> audit_mf >> quality_mf >> reconcile_mf
